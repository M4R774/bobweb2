...
@mock.patch('bobweb.bob.utils_common.fetch_json', mock_response_200_with_helsinki_weather)  # Default mock response
class WeatherCommandTest(django.test.TransactionTestCase):
    ...
    async def test_should_inform_if_city_not_found(self):
        ...
        with mock.patch('bobweb.bob.utils_common.fetch_json', mock_fetch_json_with_content({"cod": "404"})):
    ...
    async def test_known_user_no_parameter_should_reply_with_users_last_city(self):
        ...
        with mock.patch('bobweb.bob.database.get_chat_member', lambda *args, **kwargs: mock_chat_member):
            with mock.patch('bobweb.bob.utils_common.fetch_json', mock_response_200_with_turku_weather):
