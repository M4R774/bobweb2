...
async def capture_frame(stream_status: StreamStatus) -> bytes:
    ...
    # based on the stream's bitrate. However, 768 kb should be sufficient. This value should be raised
    # if malformations occur in the captured stream frames. The most common is having a clear line on
    # frame with area below it repeating last full detail vertical line to the bottom of the frame.
    stream_bytes = stream_reader.read(1024 * 768)  # Number of bytes to read
    stream_reader.close()
    await asyncio.sleep(0)  # To yield to the event loop
