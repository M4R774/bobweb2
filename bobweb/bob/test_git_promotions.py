...
from bobweb.bob import main, database, git_promotions
from bobweb.bob.tests_mocks_v2 import MockBot
...
class TestGitPromotions(TestCase):
    ...
    @mock.patch('bobweb.bob.git_promotions.promote_committer_or_find_out_who_he_is', AsyncMock())
    ...
    async def test_broadcast_and_promote_sends_message_with_expandable_quote(self, broadcast_mock: AsyncMock):
        ...
        bob.latest_startup_broadcast_message = None
        bob.save()
        ...
        bob = database.get_the_bob()
        self.assertEqual(default_commit_message, bob.latest_startup_broadcast_message)

    @mock.patch('os.getenv', lambda key: default_commit_message)
    @mock.patch('bobweb.bob.git_promotions.promote_committer_or_find_out_who_he_is', AsyncMock())
    @mock.patch('bobweb.bob.database.get_the_bob', Mock())
    @mock.patch('bobweb.bob.broadcaster.broadcast')
