...
class TestACompletion(TestCase):
    ...
    async def test_acompletion_success(self):
        ...
        with patch("bobweb.bob.litellm_utils.litellm.acompletion", new=AsyncMock(return_value=mock_response)):
    ...
    async def test_content_policy_violation_error(self):
        ...
        with patch("bobweb.bob.litellm_utils.litellm.acompletion", new=AsyncMock(side_effect=error)):
    ...
    async def test_unknown_exception(self):
        ...
        with patch("bobweb.bob.litellm_utils.litellm.acompletion", new=AsyncMock(side_effect=UnknownLLMError("Unexpected failure"))):
            with pytest.raises(ResponseGenerationException) as exc_info:
                await acompletion("test")
