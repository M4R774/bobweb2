...
async def handle_google_genai_response_not_ok(response: ClientResponse,
    ...
    log_title = f'Google GenAI API request failed.'
    ...
    if response.status == 400 and error_status == 'INVALID_ARGUMENT':
        ...
        log_title = f"Google GenAI API request body is malformed."
        log_level = logging.ERROR
    elif response.status == 400 and error_status == 'FAILED_PRECONDITION':
        error_response_to_user = 'Virhe maksutiedoissa.'
        log_title = f"Google GenAI API has problem with billing."
        log_level = logging.ERROR
        ...
        log_title = f"Google GenAI API authentication failed."
        log_level = logging.ERROR
    elif response.status == 404 and error_status == 'NOT_FOUND':
        error_response_to_user = 'Kysymyksistä tippui media matkalla.'
        log_title = f"Google GenAI API was unable to see linked media."
        log_level = logging.ERROR
        ...
        log_title = f"Google GenAI API rate limit exceeded."
        log_level = logging.INFO
        ...
        log_title = f"Google GenAI API unable to finish on time."
        log_level = logging.INFO
