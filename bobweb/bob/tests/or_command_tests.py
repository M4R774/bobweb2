...
from bobweb.bob.tests.utils import assert_has_reply_to, assert_no_reply_to, assert_reply_to_contain, assert_reply_equal
...
class Test(TestCase):
    ...
    def test_only_one_parameter_should_not_reply(self):
        assert_no_reply_to(self, 'tests /vai')
        assert_no_reply_to(self, '/vai tests')

    def test_parameter_before_and_after_should_reply(self):
        assert_has_reply_to(self, 'tests .vai tests')
