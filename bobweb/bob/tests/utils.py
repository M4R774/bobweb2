...
def assert_get_parameters_returns_expected_value(test: TestCase, command_text: str, command: ChatCommand):
    message = f'{command_text} tests . tests/tests-tests\ntests\ttests .vai tests \n '
    parameter_expected = 'tests . tests/tests-tests\ntests\ttests .vai tests'
...
def mock_response_200(*args, **kwargs) -> MockResponse:
    del args, kwargs
    return MockResponse(status_code=200, content='tests')
