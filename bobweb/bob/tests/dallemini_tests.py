...
from bobweb.bob.tests.mocks_v1 import MockUpdate
from bobweb.bob.tests.utils import assert_has_reply_to, assert_no_reply_to, assert_reply_to_contain, \
    mock_response_with_code, assert_reply_equal, MockResponse, assert_get_parameters_returns_expected_value

from bobweb.bob.command.dallemini_command import convert_base64_strings_to_images, get_3x3_image_compilation, send_image_response, \
...
class Test(IsolatedAsyncioTestCase):
    ...
    def test_text_before_command_no_reply(self):
        assert_no_reply_to(self, 'tests /dallemini')

    def test_text_after_command_should_reply(self):
        assert_has_reply_to(self, '/dallemini tests')
    ...
    def test_send_image_response(self):
        update = MockUpdate()
        update.effective_message.text = '.dallemini tests'
        prompt = 'tests'
        expected_image = Image.open('bobweb/bob/resources/tests/test_get_3x3_image_compilation-expected.jpeg')
    ...
    # Simple tests that images are similar. Reduces images to be 100 x 100 and then compares contents
