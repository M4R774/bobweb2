...
from bot import command_service, database
from bot.activities.activity_state import ActivityState
from bot.main import main
from bot.tests_mocks_v2 import init_chat_user, assert_buttons_equals, MockUpdate
from web.bobapp.models import Bot
...
class CommandServiceTest(django.test.TransactionTestCase):
    ...
    async def test_reply_and_callback_query_handler_should_delegate_button_press(self):
        ...
        with mock.patch(
                'bot.command_service.instance.reply_and_callback_query_handler',
                wraps=bot.command_service.instance.reply_and_callback_query_handler) as mock_method:
    ...
    async def test_starting_new_activity_without_host_message_logs_error_to_specified_error_log_chat(self):
        ...
        bot_from_db: Bot = database.get_bot()
        ...
        with self.assertLogs(level='WARNING') as log:
            ...
            expected_log = ("Started new CommandActivity for which its initial state did not create a host message. "
                            "InitialState: <class 'bot.activities.activity_state.ActivityState'>")
