...
class EnvTests(TestCase):
    ...
    def test_env_file_loaded_when_env_not_set(self):
        ...
        try:
            ...
            self.assertEqual('from_env_file', config.bot_token)
        finally:
    ...
    def test_env_file_does_not_override_existing_env(self):
        ...
        try:
            ...
            _write_env_file('BOT_TOKEN=from_env_file\n')

            # Reload config so it picks up .env but should not override real env
            importlib.reload(config)
            self.assertEqual('from_os_env', config.bot_token)
        finally:
            # Clean up
            try:
