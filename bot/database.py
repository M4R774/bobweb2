...
def get_bot():
    try:
        ...
        bot = Bot(id=1, uptime_started_date=datetime.now(timezone.utc))
        bot.save()
        return bot
...
def get_global_admin():
    bot = get_bot()
    return bot.global_admin


def get_credit_card_holder():
    bot = get_bot()
    return bot.gpt_credit_card_holder


def set_credit_card_holder(new_credit_card_holder: TelegramUser):
    bot = get_bot()
    bot.gpt_credit_card_holder = new_credit_card_holder
    bot.save()
